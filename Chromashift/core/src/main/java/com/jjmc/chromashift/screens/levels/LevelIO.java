...
public class LevelIO {
                ...
                try {
                    ...
                    try {
                        ...
                        if (assetsDir != null) {
                            ...
                            Gdx.app.log("LevelIO", "Copied internal level to workspace assets: " + out.getAbsolutePath());
                        }
                    } catch (Exception ex) {
                        Gdx.app.error("LevelIO", "Failed to copy internal level to workspace assets: " + ex.getMessage());
                    }
                    return s;
                } catch (Exception parseEx) {
                    Gdx.app.error("LevelIO", "Failed to parse level JSON from internal asset: " + path, parseEx);
                    try { Gdx.app.error("LevelIO", "Content preview: " + text.substring(0, Math.min(400, text.length()))); } catch (Exception ignored) {}
                }
            }
            // If internal not available or parsing failed, try the workspace assets folder as a fallback.
            java.io.File assetsDir = findProjectAssetsDir();
            if (assetsDir != null) {
                ...
                if (candidate.exists()) {
                    ...
                    try {
                        ...
                        Gdx.app.error("LevelIO", "Failed to parse level JSON from workspace asset: " + candidate.getAbsolutePath(), parseEx);
                        try { Gdx.app.error("LevelIO", "Content preview: " + text.substring(0, Math.min(400, text.length()))); } catch (Exception ignored) {}
                    }
                }
            }
    ...
    /** Save the level state to workspace assets and mirror into build resources (overwrites). */
    public static boolean save(String path, LevelState state) {
        try {
            ...
            try {
                ...
                // Also mirror to build resources so the running game/editor sees the latest file
                try {
            ...
            if (assetsDir != null) {
                ...
                if (candidate.exists()) {
                    ...
                    try {
                        ...
                        try {
                            boolean ok = writeToBuildResources(path, text);
                            if (ok) Gdx.app.log("LevelIO", "Copied workspace level to build resources: " + path);
                        } catch (Exception ex) {
                            Gdx.app.error("LevelIO", "Failed to copy workspace level to build resources: " + ex.getMessage(), ex);
                        }
                        return s;
                    } catch (Exception parseEx) {
                        Gdx.app.error("LevelIO", "Failed to parse level JSON from workspace asset: " + candidate.getAbsolutePath(), parseEx);
    ...
    /**
     * Try to discover the project's assets directory by walking up from the current working
     * directory and looking for a folder named `assets` that also contains an `assets.txt` or
        ...
        try {
            ...
            while (dir != null && depth < 8) {
                ...
                if (assets.exists() && assets.isDirectory()) {
                    ...
                    if (new java.io.File(assets, "assets.txt").exists() || new java.io.File(assets, "levels").exists()) {
                        return assets;
                    }
                }
                dir = dir.getParentFile();
